...
{ 
    ...
    {                 
      ...
      foreach($this->getOptions() as $option){
        ...
        foreach($option['values'] as $value){
          ...
          try {
          	$data['image'][$t][$valueId] = Mage::helper('catalog/image')->init($_product, 'tiny_image',$value['image'])->__toString();;
          } catch (Exception $e) {
          	$data['image'][$t][$valueId] = $value['image'];
          }
          $data['image'][$t][$valueId] = preg_replace('/.*?media\/extendware/', '', $data['image'][$t][$valueId]);
    ...
    { 	
      if (!empty($image)){
        ...
        if ($this->thumbnailDirUrl == ''){
          $this->thumbnailDirUrl = preg_replace('/extendware\/ewminify\/.*/', 'extendware', $thumbnailUrl);		
          $this->pickerImageDirUrl = preg_replace('/extendware\/ewminify\/.*/', 'extendware', $pickerImageUrl);
        }	
      }
	  }
  ...
  // Set Thumbnail Size:			
    ...
    { 			
			return Mage::app()->getStore()->getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA) . '/extendware';
	 	}	
