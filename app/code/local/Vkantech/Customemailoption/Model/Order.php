 ...
 */
class Vkantech_Customemailoption_Model_Order extends Mage_Sales_Model_Order
{
    /**
    ...
    public function sendNewOrderEmail()
    {
		...
		//echo "Order Id is".$this->getId();		
		$items = $this->getAllItems();
		foreach ($items as $item) {		
			//echo "Product ID is".$item->getProductId();
			$product = Mage::getModel('catalog/product')->load($item->getProductId());
			$options = $product->getProductOptionsCollection();
			if(count($options) > 0 ) {
				$option_arry = $item->getProductOptions();
				foreach ($options as $option) {					
					if(count($option_arry['options']) > 0){
