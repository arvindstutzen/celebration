...
$database_extension = 'mysqli'; // 'auto', 'pdo', 'mysqli', 'mysql'
...
//define('USER_TABLE_PREFIX','mg_');  	  // database prefix
...
//				$db_link = new PDO("mysql:host=" . $settings['hostname'] . ";dbname=" . $settings['database'], $settings['username'], $settings['password']);
...
function PMA_importRunQuery($sql = '', $full = '') {
	global $db_link,$import_run_buffer, $go_sql, $complete_query, $display_query, $sql_query, $my_die, $error, $reload, $finished, $timeout_passed, $skip_queries,
		   $executed_queries, $max_sql_len, $read_multiply, $cfg, $sql_query_disabled, $db, $run_query, $is_superuser, $message, $show_error_header, $file_name,
		   $checksum_sm, $prev_sql_exec, $db_flag;
...
//									$result = $db_link->query($import_run_buffer['sql']);
									if (reconnect() == 1) {
										try {
											...
											$ret .= POST_ERROR_SQL_INDEX . "|" . $cur_sql_index . '|<font color="#000000"><b>' . $e->errorInfo[1] . '; ' . $e->errorInfo[2] . '</b></font>;' . htmlspecialchars($import_run_buffer['sql']) . '<br>';
...
//											$st = mysql_error($db_link->dbconn->dbconn);
											$ret .= POST_ERROR_SQL_INDEX . "|" . $cur_sql_index . '|<font color="#000000"><b>'.$err_no.'; '.$st.'</b></font>; '.htmlspecialchars($import_run_buffer['sql']).'<br>';
											$fp = fopen($file_name, "w");
											if ($fp) {
												fputs($fp, $checksum_sm . "|" . --$cur_sql_index);
												fclose($fp);
								...
								$ret .= '<font color="#000000"><b>'.$err_no.'; '.$st.'</b></font>;'.$import_run_buffer['sql'].'<br>';
							}
						}
